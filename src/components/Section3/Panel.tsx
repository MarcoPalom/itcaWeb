...
const ANIMATION_DURATION = 600; // ms, igual a la duración de la animación de salida
...
const Panel: React.FC<PanelProps> = ({
  ...
  return (
    <div ref={ref} className="w-full min-h-screen bg-[#F5F1E8] flex flex-col justify-between">
      {/* Título con efecto glitch */}
      <div
        className={`flex items-center justify-${isLeft ? "start" : "end"} w-full px-8 pt-8 mb-2`}
        style={{
          minHeight: "3.5rem",
      ...
      >
        <div className="relative mr-4">
          ...
          <motion.h2
            className="text-3xl md:text-4xl font-extrabold tracking-tight absolute top-0 left-0 text-red-500 mix-blend-multiply z-0"
          ...
          <motion.h2
            className="text-3xl md:text-4xl font-extrabold tracking-tight absolute top-0 left-0 text-blue-500 mix-blend-multiply z-0"
          ...
          <motion.h2
            className="text-3xl md:text-4xl font-extrabold tracking-tight absolute top-0 left-0 text-green-500 mix-blend-multiply z-0"
        ...
        <div className="flex space-x-2 relative">
          {[0, 1, 2].map((i) => (
            <div key={i} className="relative">
              {/* Punto principal */}
              <motion.span
                className="inline-block w-3 h-3 rounded-full bg-black relative z-0"
              ...
              <motion.span
                className="inline-block w-3 h-3 rounded-full bg-red-500 absolute top-0 left-0 mix-blend-multiply z-0"
              ...
              <motion.span
                className="inline-block w-3 h-3 rounded-full bg-blue-500 absolute top-0 left-0 mix-blend-multiply z-0"
      ...
      <div className="flex items-start justify-between px-8 pt-6">
        {isLeft ? (
          <span className="bg-black text-white text-xs px-2 py-1 rounded font-mono">ITCA</span>
        ) : (
          <div className="flex space-x-1 text-3xl font-black">
      ...
      <div className={`flex flex-1 items-start justify-between px-8 pb-8 ${isLeft ? "" : "flex-row-reverse"}`}>
        {/* Imagen pixel art */}
        <div className={`flex flex-col ${isLeft ? 'items-start' : 'items-end'}`}>
          ...
          <span className="text-xs font-mono border border-black rounded-full px-2 py-1 mt-2 flex items-center gap-1">
            {badge}
          </span>
        </div>
        ...
        <div className={`flex flex-col items-start justify-center flex-1 ${isLeft ? "ml-16" : "mr-16"} mt-8`}>
          <h2 className="text-m font-medium mb-42 min-h-[4em]">{selected === null || selected === undefined ? (defaultDescription || "Selecciona un programa para ver la descripción.") : descriptions[selected]}</h2>
          <div className="bg-yellow-300 text-black text-base font-mono font-medium px-1 py-1 rounded mb-8 mt-8 transition-all border border-black">
            PROGRAMAS
          </div>
          <ul className="text-2xl font-medium space-y-2">
            {items.map((item, idx) => (
              <motion.li
                ...
                onMouseEnter={() => setHovered && setHovered(idx)}
              ...
              >
                ...
                <motion.span
                  className="z-0 select-text"
                  animate={hovered === idx ? { x: "1.5rem", scale: 1.04 } : { x: 0, scale: 1 }}
                  transition={{ type: "spring", stiffness: 400, damping: 12 }}
                >
                  {item}
                </motion.span>
      ...
      {/* Línea divisoria fina y redondeada */}
      <div className="w-11/12 mx-auto my-4 h-1 rounded-full bg-neutral-300" />
    </div>
  )
}
